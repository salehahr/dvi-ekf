...
import casadi
import numpy as np
from context import Camera, Imu, SimpleProbe, syms
...
class TestFilter(unittest.TestCase):
    ...
    def states(cls):
        ...
        cls.dofs, cls.ddofs, cls.dddofs = casadi.vertsplit(
            syms.dofs_cas, [0, 8, 16, 24]
        )
    ...
    def test_fun_nominal(self):
        p_B_next = (
            self.p_B + self.dt * self.v_B + self.dt ** 2 / 2 * self.R_WB @ self.acc
        )

        fun_nominal = casadi.Function(
            "f_nom",
    ...
    def _derive_err_pc_dot(self):
        ...
        R_WB_tr = self.R_WB @ (casadi.DM.eye(3) + casadi.skew(self.err_theta))
        ...
        p_CB_dot = self.R_WB @ self.v_CB + casadi.skew(self.om) @ self.R_WB @ self.p_CB
        p_CB_dot_tr = R_WB_tr @ self.v_CB + casadi.skew(om_tr) @ R_WB_tr @ self.p_CB
    ...
    def test_fun_error(self):
        ...
        fun_error = casadi.Function(
            "f_err",
            [self.dt, *self.err_x, *self.u, *self.n, self.R_WB],
            [
                self.err_p_B + self.dt * self.err_v_B,
                self.err_v_B
                + self.dt * (-self.R_WB @ casadi.skew(self.acc) @ self.err_theta)
                + self.n_v,
            ...
            ["dt", *self.err_x_str, *self.u_str, *self.n_str, "R_WB"],
            [
                "err_p_B_next",
                "err_v_B_next",
                "err_theta_next",
