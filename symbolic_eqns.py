...
f_predict = casadi.Function('f_nom',
    [sym.dt, *sym.x, *sym.u, *sym.probe_fwkin],
    [   sym.p_B + sym.dt * sym.v_B \
            + (sym.dt**2 / 2) * sym.R_WB @ sym.B_acc_BW,
        sym.v_B + sym.dt * sym.R_WB @ sym.B_acc_BW,
        sym.R_WB + sym.R_WB @ casadi.skew(sym.dt * sym.B_om_BW),
        sym.dofs,
        sym.p_C \
            + sym.dt * sym.v_B \
            + sym.dt * sym.R_WB @ (sym.BB_v_CB + \
                casadi.cross(sym.B_om_BW, sym.B_p_CB)),
        sym.R_WC_kf + sym.R_WC_kf @ casadi.skew(sym.dt * sym.C_om_CW)],
    ['dt', *sym.x_str, *sym.u_str, *sym.probe_fwkin_str],
