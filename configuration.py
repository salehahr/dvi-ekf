...
probe = SimpleProbe(scope_length=scope_length, theta_cam=theta_cam_in_rad)
...
GYRO_NOISE = 0.005 * math.sqrt(NOISE_SAMPLE_RATE)  # [deg/s]
GRAVITY = 9.81 * 100  # [cm/s^2]
ACCEL_NOISE = 400 * 1e-6 * GRAVITY * math.sqrt(NOISE_SAMPLE_RATE)  # [cm/s^2]
...
class Config(object):
    def __init__(self, traj_name=None):
        ...
        # simulation params
        self.num_kf_runs = args.runs if not do_plot_only else NUM_KF_RUNS_DEFAULT
        self.mode = args.m
        ...
        self.process_noise_rw_std = (
            np.hstack((STDEV_DOFS_R, STDEV_DOFS_P, STDEV_DOFS_NOTCHdd))
        ...
        self.img_filepath_compact = "img/kf_" + self.img_filename + "_compact.png"
    ...
    def _parse_arguments(self):
        ...
        # positional args
        parser.add_argument(
            "traj_name", type=str, help="mandala0_mono, trans_x, rot_x, ..."
        )
