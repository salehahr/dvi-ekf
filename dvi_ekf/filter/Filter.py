...
from dvi_ekf.filter.state import ErrorState, State
from dvi_ekf.kinematics import equations as eqns
from dvi_ekf.kinematics import symbols as syms
from dvi_ekf.models.Camera import Camera, CameraMeasurementArray
from dvi_ekf.models.trajectory.FilterTraj import FilterTraj
from dvi_ekf.tools import files
from dvi_ekf.tools.math import skew
from dvi_ekf.tools.Quaternion import Quaternion
from dvi_ekf.tools.utils import get_upd_values
from dvi_ekf.visuals import FilterPlot

if TYPE_CHECKING:
    from dvi_ekf.filter.Simulator import Simulator
    from dvi_ekf.models.measurement_point import CameraMeasurementPoint
    from dvi_ekf.models.trajectory.TrajectoryBase import VisualMeasurementPoint
...
class Filter(object):
    ...
    def __init__(self, sim: Simulator):
        ...
        assert self._P.shape == (ErrorState.SIZE, ErrorState.SIZE)

        self._frozen_dofs = [bool(fr) for fr in self._config.frozen_dofs]
        ...
        # static matrices
        self.H = np.zeros([self.num_meas, ErrorState.SIZE])
    ...
    def run(self, camera: Camera, k: int, run_desc_str: str) -> None:
    ...
    def run_one_epoch(self, old_t: float, t: float, i_cam: int, camera: Camera) -> None:
        """
        ...
        # update
        cam_meas = camera.traj.at_index(i_cam)  # not counting IC
        ang_notch = camera.get_notch_vec_at(i_cam)[0]
        self.update(t, cam_meas, ang_notch)

    def propagate_imu(self, t0: float, tn: float):
        ...
        """
        cam_array = CameraMeasurementArray.from_camera(self.imu.cam, t0, tn)
        real_probe_dofs = self._config.gt_joint_dofs

        old_ti = t0
        for i_cam, ti in enumerate(cam_array.t):
            cam_meas: CameraMeasurementPoint = cam_array.at_index(i_cam)
            real_probe_dofs[0][6] = cam_meas.notch
            real_probe_dofs[1][6] = cam_meas.notch_d
            real_probe_dofs[2][6] = cam_meas.notch_dd
            ...
            self.imu.ref.append_value(ti, cam_meas.vec, cam_meas.notch_arr)
    ...
    def _predict_nominal(self, om, acc):
        ...
        res = [
            casadi.DM(r).full()
            for r in eqns.f_predict(
                self._dt, *self._states.array, *self.u, *est_probe, om, acc
            )
        ]
    ...
    def _predict_error(self):
        ...
        Fi = casadi.SX.zeros(ErrorState.SIZE, self.num_noise)
    ...
    def update(self, t: float, camera: VisualMeasurementPoint, ang_notch: float):
        # compute gain
        H = self.H
        ...
        cam_rot_corrected = notch_quat * camera.q
        ...
        err = ErrorState.from_array(K @ res)
        for i, fr in enumerate(self._frozen_dofs):
            if fr:
                err.ddofs[i] = 0
        ...
        self._states += err
        I = np.eye(ErrorState.SIZE)
        self._P = (I - K @ H) @ self._P @ (I - K @ H).T + K @ self.R @ K.T

        # reset error states
        G = np.eye(ErrorState.SIZE)  # 24x24
    ...
    def save(self):
        ...
        files.save_trajectory(self.traj, kf_filepath)
        files.save_trajectory(self.imu.ref, imuref_filepath)
