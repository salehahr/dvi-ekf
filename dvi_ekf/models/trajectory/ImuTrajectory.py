...
from dvi_ekf.kinematics.transformations import to_world_coords
from dvi_ekf.tools.Quaternion import Quaternion

from .Interpolator import Interpolator
from .TrajectoryBase import TrajectoryBase
from .VisualTrajectory import VisualTraj


class ImuTraj(TrajectoryBase):
    ...
    def __init__(
        ...
        covariance: List[float] = [0.0] * 6,
        unnoised: bool = False,
    ):

        # base properties
        super().__init__(name, filepath, interframe_vals=interframe_vals)
        self.vis_data = None
    ...
    def reconstruct(self, R_WB, W_p_BW_0, WW_v_BW_0):
        ...
        W_acc = to_world_coords(R_WB, np.asarray((self.ax, self.ay, self.az)).T)
        ...
        W_om_B = to_world_coords(R_WB, np.asarray((self.gx, self.gy, self.gz)).T)
    ...
    def reconstruct_vis_traj(self):
        ...
        v0 = to_world_coords(R_WB0, np.asarray((self.vx[0], self.vy[0], self.vz[0])))
        q0 = self.vis_data.quats[0]

        # integrating for pos
        a_W = to_world_coords(R_WB_arr, np.asarray((self.ax, self.ay, self.az)).T)
