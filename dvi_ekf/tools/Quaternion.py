...
from dvi_ekf.tools.math import euler_error_checking
...
class Quaternion(object):
    ...
    def __init__(
        ...
        do_check = False
        if do_check:
            euler_error_checking(self.euler_xyz_deg, self.euler_zyx_deg)
    ...
    def _parse_inputs(
        ...
        if val is not None:
            if isinstance(val, np.ndarray):
                ...
                Quaternion.__raise_wrong_input(val)
    ...
    @staticmethod
    def about_axis(angle: np.ndarray, axis: np.ndarray) -> Quaternion:
        """
        Returns the quaternion about the given axis.
        From: https://github.com/aipiano/ESEKF_IMU/blob/master/transformations.py
        """
        ...
        xyzw = np.array([*q[1:4], q[0]])

        return Quaternion(val=xyzw, do_normalise=True)

    @staticmethod
    def __raise_wrong_input(val) -> None:
        """Raises a TypeError for the given value."""
