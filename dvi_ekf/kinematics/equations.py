...
WW_v_BW_exp = sym.WW_v_CW - R_WB_exp @ sym.BB_v_CB - W_omB_x_pCB

W_alp_BW_exp = (
    sym.W_alp_CW
    - R_WB_exp @ sym.B_alp_CB
    - casadi.cross(W_om_BW_exp, R_WB_exp @ sym.B_om_CB)
)
W_acc_BW_exp = (
    sym.W_acc_CW
    - R_WB_exp @ sym.B_acc_CB
...
""" Kalman filter stuff """
f_predict = casadi.Function(
    "f_nom",
    [sym.dt, *sym.x, *sym.u, *sym.probe_fwkin, *sym.u_current],
    [
        sym.p_B + sym.dt * sym.v_B + (sym.dt ** 2 / 2) * R_acc_avg_exp,
        ...
        sym.p_C
        + sym.dt * sym.v_B
        + sym.dt * sym.R_WB @ (sym.BB_v_CB + casadi.cross(B_om_BW_avg_exp, sym.B_p_CB)),
